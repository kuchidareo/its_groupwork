...
while step < simulation_step:
    ...
    for ev_id in soulEV65_ids:
        if ev_id not in running_vehicles:
            ...
            if status_id == CHARGING_STATUS_ID:
                continue
            else:
                print(ev_id, status_id)
                used_charging_station_id = charging_vehicles[ev_id]["charging_station_id"]
                charging_stations[used_charging_station_id]["num_waiting"] -= 1
                charging_vehicles.pop(ev_id)
        else:
            try:
